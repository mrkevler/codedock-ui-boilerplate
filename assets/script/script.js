class TutorialApp {
    ...
    bindEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (!e.target.closest('#login-panel') && 
                !e.target.closest('#login-toggle')) {
                this.closePanel('login-panel');
            }
            
            if (!e.target.closest('#signup-panel') && 
    ...
    async loadContent() {
        try {
            ...
            const mainContent = await fetch('modules/main.html').then(r => r.text());
            ...
            this.pages = {
                ...
                'core-concepts': await fetch('modules/tutorials.html').then(r => r.text()),
                'frontend-basics': await fetch('modules/tutorials.html').then(r => r.text()),
                'ui-frameworks': await fetch('modules/tutorials.html').then(r => r.text()),
                'js-ecosystem': await fetch('modules/tutorials.html').then(r => r.text()),
                'interactive-frontend': await fetch('modules/tutorials.html').then(r => r.text())
            };
    ...
    setTheme(theme) {
        ...
        document.getElementById('light-theme').classList.toggle('bg-purple-100', theme === 'light');
        document.getElementById('dark-theme').classList.toggle('bg-purple-100', theme === 'dark');
    ...
    setContrast(high) {
        ...
        // Update accessibility panel buttons
        document.getElementById('normal-contrast').classList.toggle('bg-purple-100', !high);
        document.getElementById('high-contrast').classList.toggle('bg-purple-100', high);
    ...
    copyCode(button) {
        const codeBlock = button.closest('.code-snippet').querySelector('.code-content');
...
function toggleTutorialsDropdown() {
    ...
    document.addEventListener('click', function closeDropdown(e) {
        if (!e.target.closest('#tutorials-dropdown') && !e.target.closest('button[onclick="toggleTutorialsDropdown()"]')) {
